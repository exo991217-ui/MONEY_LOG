...
const DEFAULT_DATA=()=>({
  ...
  budgetCategories:[
    ...
    {id:102,name:'생필품',budget:100000,synced:true,syncFrom:''},
    {id:103,name:'문화/여가',budget:50000,synced:true,syncFrom:''},
    {id:104,name:'카페',budget:50000,synced:true,syncFrom:''},
  ],
  monthBudgets:{},
...
function renderAssetStocks(){
  ...
  listEl.innerHTML=stocks.map(st=>{
    ...
    return `<div class="asset-stock-item">
      <div class="asset-stock-name">
        <div class="asset-stock-name-text">${st.name}</div>
        ${st.ticker?`<div class="asset-stock-ticker">${st.ticker}${st.sector?' · '+st.sector:''}</div>`:''}
      </div>
      <div style="text-align:center;font-size:11px;color:var(--text-sub);">${st.quantity}주</div>
      <div style="text-align:right;flex-shrink:0;">
        <input class="asset-stock-price-input" type="number" value="${st.currentPrice}"
          onchange="App.updateStockPrice(${st.id},this.value)"
          title="현재가 입력"/>
        <div style="font-size:10px;color:var(--text-sub);text-align:right;margin-top:2px;">원/주</div>
      </div>
      <div class="asset-stock-val">${fmt(val)}</div>
      <div class="asset-stock-rate" style="color:${positive?'var(--green)':'var(--red)'};">${positive?'+':''}${rate.toFixed(1)}%</div>
      <div class="asset-stock-actions">
...
window.App={
  ...
  openAssetModal,promptAddAssetCategory,
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  loadState();
  // Firebase 모드이면 firebase-sync.js가 로그인 후 renderAll()을 호출함
  if(!window.FB_FIREBASE_MODE){
